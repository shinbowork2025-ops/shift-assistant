...
- 連携用の出力形式には`formatVersion`（現在は`2`）を含めます。フィールドの追加は同一バージョンで行うことがありますが、既存フィールドの削除・意味変更時は必ずバージョンを上げます。取り込み側は未知のフィールドを無視してください。マスター入力CSV・Excelには適用しません。

## 入力契約：マスターCSV・Excel

従業員とシフト区分の登録・更新に使用します。画面の「Excel・CSVから登録」で取り込みます。

この入力は、店舗で従業員とシフト区分を1件ずつ手入力する負担を減らすための暫定的な補助機能です。店舗担当者が作成するファイルへ形式版の列や値は要求しません。見出し名から対応形式を判定し、対応する見出しを判定できないファイルは取込前に拒否します。未知の列は無視しますが、既存列の意味は将来も変更しません。互換性のない項目が必要な場合は、既存列を読み替えず新しい見出しを追加します。

```csv
種別,コード,名称,開始時刻,終了時刻,所属,表示順,略称,固定残業時間,シフト残業時間,実働分
従業員,E001,田中太郎,,,園芸,1,,20,,
シフト,01,早番,06:45,16:15,,,01,,0,480
シフト,休,公休,,,,,休,,0,0
```
...
| 実働分 | 使用しない | 任意。分単位の0以上の数値。シフト区分に固定する実働分で、設定した場合は勤務時間と休憩からの計算より優先 |
...
正式な人事マスター接続が安定稼働した後は、店舗向けの手動取込を廃止するか、障害時に限定した管理者用の非常手段へ縮小します。
...
| `paidMinutes` | シフト区分に固定の実働分が設定されている場合のみ数値。未設定は`null` |
| `breaks` | シフトに**実際に配置された**休憩の開始・終了時刻の配列。出力条件により、すべて勤務時間の内側に収まっていることが保証される。休日区分では常に空配列 |
| `breakMinutes` | 配置された休憩の合計分 |
| `workMinutes` | `paidMinutes`が設定済みならその固定値。未設定の場合のみ、勤務時間から配置済み休憩を差し引いた値。出力条件により休憩はツールに実装された休憩ルールを通過している |
...
- `format_version`：ツールが各行へ自動付与する出力形式の版。店舗担当者による入力は不要
