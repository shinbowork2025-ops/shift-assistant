...
- 従業員コード（社員ID）は変更されないことを前提とします。変更される可能性がある場合は、変更前後を同一人物として結び付ける不変IDを入力契約へ追加する必要があります。
- 終了時刻が開始時刻以前のシフト行（日またぎ）はエラーとして取り込みません。
- 取込によってシフトの時刻が変わった場合、配置済みの休憩が新しい勤務枠に収まるかを全日付で検査し、収まらない休憩は自動で再配置します（取込結果に件数を表示）。

### 取込前検証と適用

ファイル全体を先に検証し、追加・更新・変更なし・読込不可の件数をプレビューします。この時点では保存データを変更しません。

- エラーなし：確認後に全件を適用
- エラーあり：既定では全件中止
- エラーありで利用者が明示：正常行だけを部分適用し、読込不可行を結果へ残す
- ファイル内の従業員コード・シフトコード重複：読込不可

手作業のファイルは差分データとして扱い、ファイルに存在しない従業員を削除・無効化しません。

### 将来の人事マスター全件同期

正式採用時は、対象店舗の全在籍者を含む人事マスターへアクセスできることを前提とします。次をすべて確認できるデータだけを全件同期として扱います。
...
全件同期で見つからない従業員は物理削除せず無効化し、過去のシフト表との参照を維持します。この自動無効化と人事マスター接続は現時点では未実装です。
