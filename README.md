...
アプリ本体は公開先のサーバーから読み込みますが、入力した氏名やシフトはIndexedDBにだけ保存します。`fetch`や`XMLHttpRequest`などの送信処理は実装していません。
...
ローカルで確認する場合は、ファイルを直接開くのではなく簡易Webサーバーを使用します。
...
公開時はリポジトリのルートをGitHub Pages、Netlifyなどの静的サイトとして配信できます。ビルドコマンドは不要です。
